"""
==============================================================================
gridsearch_7.py  —  HANDMADE Grid Search per la Rete Neurale (ML-CUP 2025)
                    VERSIONE CORRETTA (no BatchNorm, MSE train / MEE report)
==============================================================================

DIFFERENZE RISPETTO ALLA VERSIONE PRECEDENTE (gridsearch_7 originale)
--------------------------------------------------------------------
[R1]  BatchNorm RIMOSSA.
       Giustificazione teorica: con N_train = 400 e batch_size tipico 16-32,
       ogni epoca produce ~12-25 batch; le statistiche di batch (mu_B,
       sigma_B) stimate su batch cosi` piccoli sono molto rumorose e
       il recovery via running-stats alla inference diventa instabile
       (Wu & He 2018, Group Normalization).  Inoltre BN + Dropout in
       cascata causa la "variance shift" di Li et al. 2019.  Per
       dataset piccoli BN toglie piu` di quanto aggiunga.

[R2]  LOSS = MSE  ->  METRICA = MEE.
       Training con `nn.MSELoss()` (stabile, gradiente smooth, risultati
       consistenti con la pratica consolidata).  Nei CSV e nei plot
       riportiamo comunque la MEE, ricalcolata on-the-fly dalla rete
       a ogni epoca (in scala originale, dopo inverse_transform di y).
       In questo modo la loss used per ottimizzare e` MSE ma il
       criterio di model selection resta aderente alla metrica della CUP.

[R3]  CRITICAL-3 FIX: `seed_everything(self.seed)` NON viene piu`
       chiamato dentro `MLCupLoader.load_and_preprocess`.  Il cup_loader
       aggiornato gia` incorpora questa correzione; qui importiamo la
       sua versione clean.

[R4]  Minor-errors fix (tutti i bias initialization & gain corretti):
        - Tanh       -> Xavier normal, gain = calculate_gain('tanh') = 5/3
        - Sigmoid    -> Xavier normal, gain = 1.0
        - ReLU/GELU/ELU/SiLU/Mish/Softplus -> Kaiming normal, nonl='relu'
        - LeakyReLU  -> Kaiming normal, a=0.01, nonl='leaky_relu'
        - SELU       -> LeCun normal (std = 1/sqrt(fan_in))
        - Output layer (lineare, nessuna att.) -> Xavier normal, gain=1.0
       Early stopping con min_delta ADATTIVO:
            min_delta = max(abs_floor, rel_delta * |best_mee|)
       (sostituisce il vecchio `min_delta=1e-4` assoluto, inefficace
        quando la MEE e` O(20)).

[R5]  MULTI-ACTIVATION + MULTI-OPTIMIZER CONDIZIONALE.
       Repliciamo l'architettura di nn_gridsearch.py:
         - ACTIVATION_REGISTRY      mappa nome -> classe PyTorch
         - OPTIMIZER_REGISTRY       mappa nome -> classe PyTorch
         - build_optimizer()        usa `inspect.signature` per passare
                                    solo i kwargs accettati (momentum,
                                    alpha, nesterov, ...), evitando
                                    TypeError quando un iperparam non si
                                    applica (es. Adam + momentum).
         - OPTIMIZER_GRID  dict di sotto-grid specifici per optimizer.

[R6]  Plot uniforme con nn_gridsearch.py:
         - Train MEE:      linea BLU continua  (#1f4ea1, '-')
         - Validation MEE: linea ROSSA tratteggiata (#c62828, '--')
       Una figura per configurazione in plots/<tag>/lc__<config_id>.png.

[R7]  Config block semplice (stile nn_gridsearch):
         - ARCHITECTURES   lista di liste [neuroni hidden]
         - COMMON_GRID     dict di iperparam comuni
         - OPTIMIZER_GRID  dict di sotto-grid condizionali
       Aggiungi/rimuovi valori nei dict per cambiare la search.

[R8]  GaussianNoise MANTENUTA (Bishop 1995: equivale a reg. L2 debole).
       Si imposta in COMMON_GRID come `'input_noise': [0.0, 0.01]`.
==============================================================================
"""
...
SCHED_FACTOR   = 0.1        # fattore riduzione LR in ReduceLROnPlateau
SCHED_PATIENCE = max(5, PATIENCE // 3)
...
# Grid COMUNE a tutti gli optimizer.
# Attivazioni ammesse: chiavi di ACTIVATION_REGISTRY
#   relu, leaky_relu, gelu, elu, selu, tanh, sigmoid, silu, mish, softplus
COMMON_GRID = {
    ...
    'dropout'     : [0.0, 0.1, 0.2],
    'batch_size'  : [32],
    'input_noise' : [0.0, 0.01, 0.05],            # 0.0 = no noise; 0.01 = Bishop 1995
}

# Grid CONDIZIONALE sugli optimizer.
# Ogni chiave qui e` un optimizer; il suo valore e` un dict di iperparam
# specifici che vengono prodotti cartesianamente coi COMMON_GRID solo
# per quell'optimizer.
OPTIMIZER_GRID = {
    # 'adamw':  {},                              # no extra hyperparam
    'adam':     {},
    # 'nadam':    {},
    # 'radam':    {},
    # 'sgd':      {'momentum': [0.0, 0.9]},     # SGD richiede momentum
    # 'rmsprop':  {'momentum': [0.0, 0.9], 'alpha': [0.9, 0.99]},
    'nesterov': {'momentum': [0.9], 'nesterov': [True]},
}


# ==============================================================================
# REGISTRIES (activation + optimizer)
# ==============================================================================
...
# ==============================================================================
# INIZIALIZZAZIONE PESI — ACTIVATION-SPECIFIC (correzione R4)
# ==============================================================================
...
def train_one_fold(X_tr_raw, y_tr_raw, X_va_raw, y_va_raw,
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        # --- Training pass (MSE su y scalato) ---
        model.train()
        for xb, yb in dl:
            optimizer.zero_grad()
...
def grid_search(X_train, y_train, configs,
    ...
    print_separator(f"HANDMADE GRID SEARCH (MSE train / MEE report)  —  tag={tag}",
                    char='#')
    ...
    with open(json_path, 'w') as fp:
        json.dump({
            ...
            'patience'       : PATIENCE,
            'loss_function'  : 'MSE (train)  ->  MEE (report/plot)',
    ...
    cols = ['short_id', 'arch_str', 'activation', 'optimizer', 'lr',
            'weight_decay', 'dropout', 'batch_size', 'input_noise',
...
if __name__ == '__main__':
    ...
    print("=" * 78)
    print("  ML-CUP 2025 — gridsearch_7.py  (MSE train / MEE report, no BN)")
    print("=" * 78)
